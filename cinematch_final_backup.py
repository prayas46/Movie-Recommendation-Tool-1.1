...
class ModernCineMatch:
    ...
    def create_main_layout(self):
        """Create main application layout - TOP NAVIGATION"""
        # Top Navigation Bar (Modern Website Style)
        self.create_top_nav()
    ...
    def create_top_nav(self):
        """Create top navigation bar - Modern Website Style"""
        nav = tk.Frame(self.root, bg='#1f1f1f', height=70)
        nav.pack(fill=tk.X, side=tk.TOP)
        nav.pack_propagate(False)
        
        # Logo
        tk.Label(nav, text="🎬 CINEMATCH", font=('Helvetica', 24, 'bold'),
                bg='#1f1f1f', fg=COLORS['primary']).pack(side=tk.LEFT, padx=30)
        
        # Nav Links
        nav_frame = tk.Frame(nav, bg='#1f1f1f')
        nav_frame.pack(side=tk.LEFT, padx=40)
        
        for text, cmd in [("Discover", self.show_emotional_match_page),
                          ("Browse", self.show_browse_page),
                          ("My Profile", self.show_profile_page),
                          ("About", self.show_about_page)]:
            btn = tk.Label(nav_frame, text=text, font=('Helvetica', 14),
                          bg='#1f1f1f', fg=COLORS['text'], cursor='hand2', padx=15)
            btn.pack(side=tk.LEFT)
            btn.bind('<Button-1>', lambda e, c=cmd: c())
            btn.bind('<Enter>', lambda e: e.widget.config(fg=COLORS['text_secondary']))
            btn.bind('<Leave>', lambda e: e.widget.config(fg=COLORS['text']))
        
        # Stats
        tk.Label(nav, text=f"📚 {len(self.data_manager.movies_df)} Movies Available",
                font=('Helvetica', 11), bg='#1f1f1f', fg=COLORS['text_secondary']).pack(side=tk.RIGHT, padx=30)
    ...
    def show_emotional_match_page(self):
        """Emotional Match Page - MODERN WEBSITE STYLE"""
        self.clear_content()
        ...
        # HERO BANNER (like app.py)
        hero = tk.Frame(scrollable_frame, bg=COLORS['card_bg'])
        hero.pack(fill=tk.X, pady=(0, 30))
        
        hero_content = tk.Frame(hero, bg=COLORS['card_bg'])
        hero_content.pack(expand=True, pady=80)
        
        tk.Label(hero_content, text="🧠 Discover by Mood",
                font=('Helvetica', 48, 'bold'), bg=COLORS['card_bg'], fg=COLORS['text']).pack(pady=20)
        tk.Label(hero_content, text="Tell us how you're feeling and we'll find your perfect movies",
                font=('Helvetica', 18), bg=COLORS['card_bg'], fg=COLORS['text_secondary']).pack(pady=10)
    ...
    def show_browse_page(self):
        ...
        # Scrollable container - FULLY FIXED
        canvas = tk.Canvas(self.content_frame, bg=COLORS['bg'], highlightthickness=0)
        scrollbar = ttk.Scrollbar(self.content_frame, orient="vertical", command=canvas.yview)
        scrollable_frame = tk.Frame(canvas, bg=COLORS['bg'])
        
        # Fix: Make scroll frame fill canvas width dynamically
        def configure_scroll(e):
            canvas.configure(scrollregion=canvas.bbox("all"))
            # Set the width of the frame to match canvas width
        ...
        # Pack canvas first to get proper width
        canvas.pack(side="left", fill="both", expand=True)
        scrollbar.pack(side="right", fill="y")
        
        # Force update to get canvas width
    ...
    def apply_browse_filters(self, parent):
        ...
        # Update scroll region after cards are added
